...
The next Sections will provide simple examples of application of some SM techniques in the \proglang{R} environment (R Development Core Team, 2014) by using the functions in \pkg{StatMatch} (D'Orazio, 2014).  These examples will use artificial data set that come with \pkg{StatMatch}; these artificial data sets are generating by considering the variables usually observed in the EU-SILC (European Union Statistics on Income and Living Conditions) survey (for major details see \pkg{StatMatch} help pages).
...
When response and predictors are all categorical, then Chi-square based association measures (Cramer's $V$) or \textit{proportional reduction of the variance} measures can be considered.  The function \code{pw.assoc} in \pkg{StatMatch} provides some of them.
...
The results in terms of average width of the cells bounds lead to conclude that all the variables being considered should be used in the SM application while the second choice, with a slight worsening of the average width of intervals, confirm the finding of the previous analyses.
...
The final component of the output list provided by \code{Frechet.bounds.cat} summarizes the uncertainty by means of the average width of the unconditioned bounds, the average width of the bounds obtained by conditioning on $X_D$ and the overall uncertainty measured as suggested by Conti \emph{et al.} (2012) (see Section \ref{sec:mtc_vars}).  Please note the it would be preferable to derive the uncertainty bounds after the harmonization of the joint distribution of the $X_D$ variables in the source data sets.
...
\item[] Berkelaar M. and others (2011) \dQuote{lpSolve: Interface to Lpsolve v. 5.5 to solve linear--integer programs}. R package version 5.6.8. \url{http://CRAN.R-project.org/package=lpSolve}
...
\item[] D'Orazio M. (2014) \dQuote{StatMatch: Statistical Matching}. R package version 1.2.2. 
...
\item[] Meyer D., Buchta C. (2014) \dQuote{proxy: Distance and Similarity Measures}. R package version 0.4-12.  \url{http://CRAN.R-project.org/package=proxy}
...
\item[] R Development Core Team (2014) \emph{R: A language and environment for statistical computing. R Foundation for Statistical Computing}, Vienna, Austria.  \url{http://www.R-project.org/}.
